...
// on point-clouds with extreem cases.
// see the skel2.txt text-file.
...
static const int NUM_POINT_BLOCKS = 1024;
...
//	assert(cpt0 && cpt1 && cpt2);	
	...
	VectorCopy(&polygon->norm, &plane->norm);
	return polygon;
}
...
int Iterate3DConvexHullFinder(void)
{
	...
	do
	{
		...
		//tmp...
		if(SideOfPlane(&plane, e->vtx[0]) != ON_PLANE)
		{
			int b = 0;
		}
		if(SideOfPlane(&plane, e->vtx[1]) != ON_PLANE)
		{
			int b = 0;
		}
...
void Print3DConvexHull(FILE *fp)
{
	...
	for(polygon = (Polygon *)PolygonList.head; polygon; polygon = polygon->next)
	{
		// Print out a tessolated version of the polygon.
		// This works because we know that the vertices
		// are ordered.  It does not, however, pick the
		// best possible tessolation.		
		for(k = 0; k + 2 < polygon->num_vertices; k++)
		{
			fprintf(fp, "\t{");
			fprintf(fp, "%d, ", int(polygon->vtx[0] - pt_cloud));
			fprintf(fp, "%d, ", int(polygon->vtx[k+1] - pt_cloud));
			fprintf(fp, "%d", int(polygon->vtx[k+2] - pt_cloud));
			fprintf(fp, "},\n");
			count++;
		}		
	}
	fprintf(fp, "};\n");
	fprintf(fp, "tri count = %d\n", count);
}
